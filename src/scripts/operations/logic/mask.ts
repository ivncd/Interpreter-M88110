...
export default class Mask extends Operation{
    static EXTENSIONS : { [extension : string] : (operands: Operand[]) => number } = {
        "" : (operands) => Mask.default(operands),
    };
    ...
    public static default(operands: Operand[]): number {
        let value = (operands[1].get() & 0xFFFF) & operands[2].get(false);

        return value;
    }

    public execute(operands: Operand[]): void {
        const func = Mask.EXTENSIONS[this.extension];
